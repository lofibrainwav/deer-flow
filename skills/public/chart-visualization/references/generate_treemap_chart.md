...
## 功能概述
以嵌套矩形展示层级结构及各节点권重，适合资产占比、市场份额、目录容量等。

## 输入字段
### 必填
- `data`: array<object>，节点数组，每条含 `name`（string）与 `value`（number），가递归嵌套 `children`。
...
## 使用建议
확保每个节点 `value` ≥0，并与子节点之和一致；树层级不宜过深，가按需要提前聚合；为提升가读성가在节点名中가上数值单位。 
