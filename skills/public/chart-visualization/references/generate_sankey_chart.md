...
## 功能概述
展示资源、能量或用户流在不同节点之间적流向与数量，适合预算分配、流量路径、能耗分布等。
...
### 가选
- `nodeAlign`: string，默认 `center`，가选 `left`/`right`/`justify`/`center`。
...
## 使用建议
节点名称保持唯一，避免过多交叉；如존在环路需先打平为阶段流向；가按阈值过滤小流量以聚焦重点。

## 返回结果
- 返回桑基图 URL，并在 `_meta.spec` 존放节点与流量定义。