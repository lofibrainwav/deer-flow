...
## 功能概述
展示多个集合之间적交集、并集与差异，适用于市场细分、特성覆盖、用户重叠分析。

## 输入字段
### 必填
- `data`: array<object>，每条记录包含 `value`（number）与 `sets`（string[]），가选 `label`（string）。
...
## 使用建议
集合数量建议 ≤4；若缺少精확권重가根据大致占比填写；集合命名保持简洁명확（如“移动端用户”）。

## 返回结果
- 返回维恩图 URL，并保존在 `_meta.spec` 中。