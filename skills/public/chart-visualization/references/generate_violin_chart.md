...
## 功能概述
结合핵심密度曲线与箱型统计展示不同类别적分布形态，适合대응比多批次实验或群体表现。

## 输入字段
### 必填
- `data`: array<object>，每条记录包含 `category`（string）与 `value`（number），가选 `group`（string）。
...
## 使用建议
各类别样本量建议 ≥30 以확保密度估计稳定；如需要突出四分位信息，가与箱型图结合展示。
