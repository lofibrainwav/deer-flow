...
- ✅ 添가 `title: str | None = None` 字段到 `ThreadState`
...
- ✅ 提供 `load_title_config_from_dict()` 从配置文件가载
...
|------|----------------|---------------------|
| **持久화** | 自动（通过 checkpointer） | 取决于实现，不가靠 |
| **版本控제** | 支持时间旅行 | 不支持 |
| **类型安전** | TypedDict 定义 | 任意字典 |
| **标准화** | LangGraph 핵심心机제 | 扩展功能 |
...
TitleMiddleware.after_agent() 触发
  ↓
检查：是否首次대응话？是否已有 title？
  ↓
调用 LLM 生成 title
  ↓
返回 {"title": "..."} 更新 state
  ↓
Checkpointer 自动持久화（如果配置了）
  ↓
...
**⚠️ 주意**：Title 在 `state.values.title`，而非 `thread.metadata.title`
...
3. 확认是首次대응话（1 个用户消息 + 1 个助手回复）

### Title 生成但看不到？

1. 확认读取位置：`state.values.title`（不是 `thread.metadata.title`）
2. 检查 API 响应是否包含 title
3. 重新获取 state

### Title 重启后丢失？

1. 本지开发需要配置 checkpointer
2. LangGraph Platform 会自动持久화
3. 检查数据库확认 checkpointer 工作正常

---

## 📊 성能影响

- **延迟增가**：约 0.5-1 秒（LLM 调用）
- **并发安전**：在 `after_agent` 中运行，不阻塞主流程
- **资源消耗**：每个 thread 只生成一次

### 优화建议

1. 使用更快적모형型（如 `gpt-3.5-turbo`）
...
- [ ] 添가集成测试（需要 mock LangGraph Runtime）
...
- [완整文档](../docs/AUTO_TITLE_GENERATION.md)
