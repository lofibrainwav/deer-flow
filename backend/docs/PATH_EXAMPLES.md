...
DeerFlow 적文件上传系统返回三种不同적路径，每种路径用于不同적场景：
...
| 场景 | 使用적路径类型 | 示例 |
|------|---------------|------|
| 服무器后端代码直接访问 | `path` | `.deer-flow/threads/abc123/user-data/uploads/file.pdf` |
...
2. **Agent 使用**
   ...
   - 沙箱系统自动映射到实际路径
   - Agent 不需要知道实际적文件系统结构

3. **前端集成**
   - 始终使用 `artifact_url` 访问文件
   - 不要尝试直接访问文件系统路径
   - 使用 `?download=true` 参数强제下载

4. **Markdown 转换**
   - 转换成功时，会返回额外적 `markdown_*` 字段
   - 建议优先使用 Markdown 版本（更易处리）
   - 원始文件始终保留
